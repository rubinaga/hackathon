...
import com.example.EcoHack.exceptions.ErrorResponse;
...
import com.example.EcoHack.exceptions.to_delete.ErrorResponse__;
    ...
    public ResponseEntity<Object> handleConstraintViolation(TransactionSystemException ex, WebRequest request) {
        ...
        if (ex.getCause() instanceof RollbackException) {
            ...
            if (rollbackException.getCause() instanceof ConstraintViolationException) {
                return new com.example.EcoHack.exceptions.to_refactor.ConstraintViolationException(HttpStatus.BAD_REQUEST, ex.getMessage()).toResponseEntity();
            }
        }
...
//        return new CustomTypeMismatchException();
        if (ex instanceof MethodArgumentTypeMismatchException ex_) {
            return new com.example.EcoHack.exceptions.api.customExceptionHandlerBaseExceptions.MethodArgumentTypeMismatchException(ex_).toResponseEntity();
        }
        // TODO possible check for ConversionNotSupportedException

        return new com.example.EcoHack.exceptions.api.customExceptionHandlerBaseExceptions.TypeMismatchException(ex).toResponseEntity();
    }
